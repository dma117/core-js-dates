...
function getNextFriday(date) {
  const fridayIndex = 5;
  const daysInWeek = 7;
  let daysLeft = date.getUTCDay() - fridayIndex;
  if (daysLeft < 0) {
    daysLeft = Math.abs(daysLeft);
  } else {
    daysLeft = daysInWeek - daysLeft;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (
    (new Date(dateEnd).getTime() - new Date(dateStart).getTime()) / msPerDay + 1
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= getCountDaysInMonth(month, year); i += 1) {
    ...
    if (dayIndex === saturdayIndex || dayIndex === sundayIndex) {
      numberOfWeekends += 1;
    }
  }
...
function getWeekNumberByDate(date) {
  ...
  const daysInWeek = 7;
  const daysInDate = getCountDaysOnPeriod(
    startDate.toISOString(),
    date.toISOString()
  );
  const mondayIndex = 1;
  let daysPriorFirstMonday = 0;
  while (startDate.getDay() !== mondayIndex) {
    startDate.setDate(startDate.getDate() + 1);
    daysPriorFirstMonday += 1;
  }
  const weeksPriorFirstMonday = daysPriorFirstMonday > 0 ? 1 : 0;
  return (
    Math.ceil((daysInDate - daysPriorFirstMonday) / daysInWeek) +
    weeksPriorFirstMonday
  );
}
...
function getNextFridayThe13th(date) {
  const searchDate = 13;
  const fridayIndex = 5;
  while (date.getDate() !== searchDate || date.getDay() !== fridayIndex) {
...
function getQuarter(date) {
  ...
  let quarter =
    Math.ceil((date.getMonth() + 1) / monthsInQuarter) % quarterCount;
  if (quarter === 0) {
    quarter = 4;
  }
  return quarter;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const workingDates = [];
  const [startYear, startMonth, startDay] = period.start.split('-').reverse();
  const [endYear, endMonth, endDay] = period.end.split('-').reverse();
  const startDate = new Date(startYear, startMonth - 1, startDay);
  const endDate = new Date(endYear, endMonth - 1, endDay);
  let workingDays = 0;
  const currentDate = startDate;
  while (currentDate <= endDate) {
    if (workingDays < countWorkDays) {
      const workingDate = new Intl.DateTimeFormat('ru', {
        day: 'numeric',
        month: 'numeric',
        year: 'numeric',
      }).format(currentDate);
      workingDates.push(workingDate.split('.').join('-'));
